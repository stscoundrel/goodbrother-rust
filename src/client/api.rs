...
pub fn parse_request_url(username: &str, api_url: &str, page: i8) -> String {
    format!("{api}/search/issues?q=user:{user}+is:pr+state:open+archived:false&per_page=100&page={page}",
        api = api_url,
        user = username,
        page = page)
}
...
mod tests {
    ...
    fn parses_request_url() {
        ...
        assert_eq!(result1, "https://api.github.com/search/issues?q=user:stscoundrel+is:pr+state:open+archived:false&per_page=100&page=1");
        assert_eq!(result2, "https://api.github.com/search/issues?q=user:someoneelse+is:pr+state:open+archived:false&per_page=100&page=2");
    }
}